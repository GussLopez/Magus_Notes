...
const Notas = () => {
  ...
  const footerRef = useRef<HTMLDivElement>(null); // Ref del footer
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIconFixed(!entry.isIntersecting); // Fijo si el footer no está visible
      },
    ...
    <>
      ...
      <div className='max-w-screen-lg mx-auto px-2 h-screen'>
        ...
        {filtrar.length === 0 ? (
         ...
         <a
          href="/crearNota"
          className={`left-0 right-0 ${iconFixed ? 'fixed bottom-5' : 'absolute bottom-[-150px]'}`}
          style={{
